...
#Step II: Define a function to store the frequency and positions for that frequency in ind.
def FasterSymbol(Genome, symbol):
    ind = {}
    n = len(Genome)
    ...
    ind[0] = SymbolCount(Genome[0:n//2],symbol) #We first define the 0th element of the dictionary and use the count function to get the number of bases in that window
    for i in range (1,n):
        ind[i] = ind[i-1] #Ask to start be assigning current val being evaluated to the previous val
