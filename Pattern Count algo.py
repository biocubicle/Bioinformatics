...
def PatternCount(sequence,pattern): #def allows us to define a function (in this case called PatternCount (with function parameter sequence and pattern - input to the function))
    ...
    for i in range(len(sequence)-len(pattern)+1): # according to this statement, for a position i in the sequence from starting position, which is zero in python to (e.g., if an Oric_seq has ten bases and pattern has 3), the endpoint of 8. This is done as from the 8th to the 10th position is the last reading frame of 3-mer; after that, shifting again to the 9th position will not yield a 3-mer sequence, as a result of which we might get an error.
